...
| | |
|---|---|
| **Source** | [NHS England RTT Waiting Times](https://www.data.gov.uk/), official open dataset |
| **Period** | June 2026 |
...
Full column-by-column definitions are in [`docs/NHS_RTT_Project_Documentation.docx`](docs/NHS_RTT_Project_Documentation.docx).
...
Seven checks were run against the imported data before any KPI was trusted. Every result below is backed by an executable query in [`sql/data_quality_checks.sql`](sql/data_quality_checks.sql).
...
Full SQL for each KPI: [`sql/kpi_queries.sql`](sql/kpi_queries.sql). DAX measures: [`dax/measures.dax`](dax/measures.dax).
...
![Provider League Table](images/provider_league_table.png)
